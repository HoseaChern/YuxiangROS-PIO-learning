...
- [两轮自平衡车笔记](#两轮自平衡车笔记)
  ...
  - [转向环](#转向环)
    - [5. 控制理论](#5-控制理论)
      ...
      - [5.2 差模叠加](#52-差模叠加)
      - [5.3 转向控制律（单一完整转向环）](#53-转向控制律单一完整转向环)
...
- **扭矩快速估算**：所需堵转扭矩 $\approx$ 整车重量 $\times$ 车轮半径（`kg·cm` 直接用 kg 与 cm 相乘）。例如整车 1 kg、轮半径 3 cm → 约 3 kg·cm（建议堵转扭矩最好超过这个值）；
...
- 1:35 档 **空载 170 rpm、额定扭矩 0.82 kg·cm、堵转 2.8 kg·cm** 。按 0.4 理想下限（约 0.77 kg·cm）看似够，但按社区判据"车重 × 轮半径 ≈ 3.3 kg·cm"则明显不足——堵转扭矩储备只到临界线附近、无裕量。此类配置的典型表现是：小幅扰动能稳住，但被大一点的力一推就救不回（对照 0.4.4 的"$\theta_0 > \theta_{max}$"分支）；
...
//   θ_cmd  目标转角 (开环指令, 无指令=0); 指令项 kp·θ_cmd 驱动转向 (社区 Kp 项)
//   ωz     偏航角速度 Gyro_Z; 阻尼项 −kd·ωz 纯 D (社区 Kd 项, 无指令时单独作用 = 走直线抑制)
//   无积分, ki 恒 0 占位 (转动不累计误差; 角速度阻尼本质即积分器, 无需 I)
...
式中 $K_d/K_p$ 为折算系数（量纲 deg/(deg/s)=s）：遥控"期望多大角速度"被折算为"等效多大的目标转角指令"，使稳态转向角速度 $\approx \omega_{z,set}$，遥杆手感与旧角速度伺服一致；同时保留同一环的阻尼项 $-K_d\,\omega_z$。
...
本章集中收编各环的调参方法（极性判据、整定顺序、现象速查），参考社区平衡车串级 PID 调参经验（机械中值 → 直立环 → 速度环 → 转向环的整定顺序与现象判断法）总结而成，结论经 **多个社区来源交叉确认** 。注意速度环有一处 **结构差异导致的判据分歧** ：社区主流为"输出叠加式"（速度环输出直接叠加入电机 PWM），本仓库为"串级目标角式"（输出改写直立环目标角），故速度环极性测试现象相反，详见速度环整定小节"结构澄清"。
...
- **记录** ：本车直立环实测记录待补充，格式（现象 → 参数 → 结果）按本小节"观测与记录"条。
- **（可选）工程妥协** ：若纯 P（Kd=0）下振荡发散过快、车体频繁拍倒撑不到临界，可先给约最终估值 1/10 的极小 Kd"保命"再升 Kp；代价是阻尼使临界点后移、观测到的临界 Kp 略偏大，属可接受的折中，定稿前记得将该 Kd 归零复测一次。

### 速度环整定

速度环作为外环，通过倾角调节使 $v \to v_{set}$ 收敛，是标准的负反馈：以 $v_{set} > 0$ 起步为例，误差 $e = v_{set} - v > 0$ 使 $\mathrm{output} > 0$，代入后直立环目标角 $\theta_0 - \mathrm{output} < \theta_0$，车体 **前倾加速** 、轮子追上车身；接近目标速度后误差减小、目标角回到中值，车体回正匀速前进——全程收敛。需注意：平衡车调的是 **倾角** 而非 PWM，故轮速变化表现为"先超调后收敛"，这与"越跑越快直至倾倒"的正反馈发散有本质区别，勿将前者误判为极性反。

**结构澄清（先读，避免被社区教程带偏极性判据）** ：社区教程（STM32 平衡车主流做法）速度环多为 **输出叠加式** ——速度环输出以 PWM 形式直接与直立环输出叠加进电机，此时相对轮速呈"正反馈"外观，故其极性判据是"手拨一轮 → 两轮 **同向加速** 直至饱和 = 极性正确"。本仓库为 **串级目标角式** ：速度环输出是期望角度增量（deg），以 `target_angle = theta_0 - speed_output` 改写直立环目标角（见 3.3 公式③），闭环后对轮速是标准 **负反馈** ，极性判据恰好相反——"手拨轮应 **被抑制** 趋于停止"。两者只是结构差异，无对错之分，但 **极性测试现象相反** ，网上"拨轮应同向加速"的判据不可直接照搬本仓库。

按"先极性、后大小，先 Kp、后 Ki"的顺序，全程手扶、一次只动一个参数，前置条件：直立环已整定稳定（见上节），`SPEED_KI` 保持与 `SPEED_KP` 的 1/200 联动（`config.h` 中已内置 `SPEED_KI = SPEED_KP * 0.005`），除非消静差需要否则不单独调 Ki。操作序列总览： **编码器相位校验 → v_set=0 手拨轮验 Kp 极性 → 升 Kp 至原地收敛 → 设目标速度按 'w' 验运动收敛 → （如漂移）单独升 Ki** 。观测以串口状态行 `motion`/`speed`/`target` 为准（10Hz 周期打印，固件命令见 4.2），本节给出原理与判据。

- **编码器相位校验（极性测试前必须做）** ：手拨其中一轮再松手，串口 `speed` 读数方向应随拨动方向变化。若拨动轮子但 `speed` 恒为 0 或两轮读数互相抵消（经典现象：拨轮后速度显示不变、车毫无反应），是**编码器 A/B 相接反** （两轮速度做和恰为 0，社区经验一致），应反接该轮编码器 A/B 相后复测，勿当成调参问题。
- **Kp 极性（v_set=0 原地判）** ：默认目标速度 0 且运动关闭时，`'s'` 武装起控 → 手拨其中一个轮子，速度环应 **抑制** 它趋于停止（轮子应逐渐停下而非加速）；若轮子不减速反而加速、越转越快（正反馈发散特征），先复核编码器相位（上条），仍反则极性错。极性正确后再按 `'w'` 开启运动（设定正 $v_{set}$），轮子应稳定在目标速度附近小范围摇摆后收敛（本固件前进时 `speed` 应为正）。
- **Kp 大小（先在原地调，再带运动验证）** ：按本文档量纲 `deg/(mm/s)`，从 0.5~1 起步逐档增大（`SPEED_OUTPUT_LIMIT=10°` 时约 10~30 mm/s 误差即饱和，见下"注意积分饱和"）；勿直接照搬社区"从 10 起"（那是按 PWM 或位置环量纲的经验值）。每档先在 v_set=0 下验证原地直立不漂移、手拨轮能快速回停，再设小目标速度（如 `'+'` 两次 = 20 mm/s）按 `'w'` 验证起步收敛：
  - 起步瞬间车体应前倾、`speed` 上升并在目标附近收敛；若速度不收敛或超调过大，减小；
  - 角度稳但 `speed` 一直上升不收敛（直线冲刺）→ Kp/Ki 不足，增大。
- **Ki（默认联动，仅漂移时单独调）** ：社区经验 $K_i' \approx K_p'/200$ 用于消静差（Kp 项只能消除即时速度误差，机械中值偏移、地面坡度、左右差异造成的 **缓慢匀速漂移** 必须靠 I 项持续修正），默认随 Kp 联动即可，无需单独整定；若直立稳定但车体缓慢漂移不回原位、或匀速运动时实际速度与设定存在稳态偏差，可单独上调 Ki 观察漂移是否收敛，出现来回甩则回落。
- **现象判定（速度环档位速查）** ：
...
- **记录** ：速度环实测记录待补充，格式按直立环整定"观测与记录"条（现象 → 参数 → 结果）。

### 转向环整定

社区通用做法是 **PD 合一转向环** ，其 Kp 与 Kd 极性判断 **彼此相反** （区别于直立环两者同向），且调节时目标值设置不同——调 Kp 时给期望角速度（示例 10）、调 Kd 时期望必须为 0（本就不期望旋转）。本仓库合并后的单一转向环 $\Delta = K_p\,\theta_{cmd} - K_d\,\omega_z$ 即社区 PD 合一环的两项： **阻尼项 $-K_d\,\omega_z$ = 社区 Kd 项** （`TURN_KD`）、 **指令项 $K_p\,\theta_{cmd}$ = 社区 Kp 项** （`TURN_KP`），同一组三元组、无模式切换。整定按"先极性、后大小，先 Kd、后 Kp"的顺序，全程手扶、一次只动一个参数。操作序列总览： **Kd 验证（无指令直行，`TURN_KD` 从 0 起调观察走直线）→ 差速符号校验 → Kp 步进转角观察（`TURN_KP`）→ 注意开环转角固有代价** 。逐项说明如下：

- **Kd（阻尼项，对应社区 Kd 项，验证时无指令 $\theta_{cmd}=0$）** ：目标转角为 0 直行。 **极性正确 = 用手转动小车，小车抑制转动（反向阻尼）** 。验证流程：`TURN_KD` 从 0 起调，未武装按 `'+'` 设一个非零前进速度 → `'s'` 武装 → `'w'` 开启运动直行，观察串口 `omega_z`——自发偏航越明显则 `TURN_KD` 需越大，调到车体能稳定走直线且不振荡为止；
- **差速符号校验**：发 `'r'`，若车体实际左转则 `'l'`/`'r'` 符号写反，调换 `main.cpp` 中 `'l'`/`'r'` 分支的加减号即可（勿改电机接线）；
- **Kp（指令项，对应社区 Kp 项，验证时有指令 $\theta_{cmd}\neq 0$）** ：发 `'l'`/`'r'` 转角指令。 **极性正确 = 用手转动小车，小车帮助转动（同向助力）** ——即 `'r'` 应右转；验证时以 `'l'`/`'r'` 步进转角，观察转向响应快慢——`TURN_KP` 过小转向迟缓、过大则车体抖动或失衡；`TURN_PWM_LIMIT` 是差速安全上限，先保守再放开；
