...
- **Kp 大小**：从小到大增大。车体能明显回正不再软塌塌时为首档；继续增大至出现**大幅低频抖动**，记录该临界值。
- **Kd 极性**：单测 Kd 现象与 Kp 一致——向哪边倾倒，轮子向哪边转动；反之为极性错误。
- **Kd 大小**：在已整定的 Kp 基础上从小到大增加，直到出现**高频震荡**（电机高频尖叫，抗干扰强但极易损坏电机），立即断电，记录临界值。
- **社区经验系数**：临界 Kp、Kd 各乘 **0.6** 作为最终参数（示例：Kp 550→330，Kd 2.5→1.5），留出稳定裕度防止临界震荡。本车实测记录见 2.5。
