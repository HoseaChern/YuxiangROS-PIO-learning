...
背景：这些常量原本以 `constexpr` 形式重复硬编码在 14 个固件内部，改引脚/换 IP 会修改
...
| `net_boot.h`       | 网络自举入口：按 `WIFI_ROLE_AP` 在 STA 接入 / AP 自组网间建网并注册 transport（含 `wifi_role_boot`、`set_microros_wifi_ap_transports`） |
...
| 网络配置   | `AGENT_IP_STR`、`AGENT_PORT`                                                                                                                                                     | micro-ROS Agent 地址（值随 `WIFI_ROLE_AP` 取 STA/AP 两套，见模板注释） |
| 网络拓扑   | `WIFI_ROLE_AP`、`WIFI_AP_SSID`、`WIFI_AP_PASS`、`WIFI_AP_CHANNEL`                                                                                                                | STA 接入 / AP 自组网开关与 SoftAP 参数                                 |
...
>
> 网络拓扑开关与 SoftAP 参数（`WIFI_ROLE_AP`/`WIFI_AP_*`）以及 `net_boot.h` 的接入契约说明见 `docs/Network_Setup_Notes.md` 第 8 节。
...
- **单一事实来源**：同一常量全仓库只有一处定义，消除 14 个固件中的重复声明；
