...
收纳 `src/main.cpp` 与 `src/tests/`(test01 至 test07)各固件共用的"环境相关、经常调整"的常量：

- **硬件接线**：电机/编码器引脚；
- **标定参数**：PID 系数、运动学轮距与脉冲当量、默认目标速度；
- **部署环境**：WiFi 凭据、micro-ROS Agent 的 IP 与端口；
- **通用参数**：串口波特率、控制周期、单位换算、任务/发布/时间同步、话题与节点名。

背景：这些常量原本以 `constexpr` 形式重复硬编码在 8 个固件内部，改引脚/换 IP 会修改
...
- **单一事实来源**：同一常量全仓库只有一处定义，消除 8 个文件中的重复声明；
