...
bool create_entities() {
    ...
    // 6. 里程计发布者: 发布 nav_msgs/Odometry 到 ODOM_TOPIC (frame_id 已在 micro_ros_task 填充)
    ...
    // 7. 时间同步: 位姿时间戳取自 rmw_uros_epoch_millis(), 须先与 Agent 对齐 epoch。
    ...
    // 8. 初始化里程计定时器: 每 ODOM_PUBLISH_MS 触发一次 odom_callback, 第三参数单位要求是 ns
 ...
 *
 * 生命周期: 网络自举 -> 填充消息 frame_id -> create_entities -> spin -> 解除武装
 * -> destroy_entities -> 延时重建, 任务永不返回。
...
void micro_ros_task(void* param) {
    ...
    // 3. 填充里程计消息的 frame_id/child_frame_id (与 test08 一致): 字符串由
    //    micro_ros_string_utilities_set 分配堆内存, 消息对象 pub_msg 跨重建循环存活,
    //    故只分配一次, 避免每次重建重复分配
    ...
    for (;;) {
        ...
        // 手动 spin 轮询: 等待集含 /cmd_vel、/balance_enable 两个订阅句柄与 odom 定时器句柄,
        // rcl_wait 单轮最多阻塞 RCL_MS_TO_NS(10), 若无事件则到点返回;
        // 仅当 agent 会话失效 (context 无效) 时 spin_some 返回 RCL_RET_ERROR
