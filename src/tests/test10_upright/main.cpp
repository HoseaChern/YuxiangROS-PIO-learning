/**
 * @file main.cpp
 * @brief test10_upright: 两轮自平衡直立环固件 (阶段一: 仅直立环 PD)
 *
 ...
 * 编译/烧录: pio run -e test10_upright -t upload --no-monitor
 */
 ...
 */
void control_step() {
    // 1. 更新姿态: theta = 陀螺仪返回实际角度, omega = 陀螺仪返回实际角速度
    mpu.update();
    const float theta = mpu.getAngleY(); // theta: 当前角 (deg)
    const float omega = mpu.getGyroY();  // omega: 当前角速度 (deg/s)
